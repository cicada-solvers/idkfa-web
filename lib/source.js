...
const Source = (() =>
{
     ...
     */
    const reverseDeep = x => Util.isArray(x) ? x.reverse().map(y => reverseDeep(y)) : x;
     ...
     * @param      {boolean}  isReversedSource  Indicates if source should be reversed
     * @return     {array}  The hierarchical source
     */
    const getSource = (arrSourcePaths, isPatchedSource = true, isReversedSource = false) =>
    {
        ...
        return arrBlobs.length > 0 ? isReversedSource ? reverseDeep(arrBlobs) : arrBlobs : null;
    };

    return { bfs, fs, getSource };
